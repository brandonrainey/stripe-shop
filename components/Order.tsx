    ...
    <div className=" flex flex-col w-11/12 sm:w-3/4 h-62 border shadow-lg rounded-lg">
      <div className="flex w-full h-20 bg-[#f6f6f6] p-1 sm:px-2 relative">
        <div className="flex flex-col mr-4 sm:mr-12 self-center">
          <p className="font-semibold text-[0.8rem] sm:text-[1rem]">ORDER PLACED</p>
        ...
        <div className="flex flex-col self-center">
          ...
          <p className="text-[0.75rem] sm:text-[0.9rem] whitespace-nowrap">{`$${order.amount}`}<span className='sm:block hidden'>{` - Next-Day Delivery $${order.amountShipping}`}</span></p>
        </div>
      ...
      <ScrollContainer className="flex p-2 px-4 gap-8 sm:h-44 h-32 items-center overflow-x-scroll scrollbar-thin scrollbar-thumb-blue-600 scrollbar-track-blue-300 scrollbar-thumb-rounded-full scrollbar-track-rounded-full" hideScrollbars={false}>
        {order.images.map((image: string, index: number) => (
          <img src={image} className='h-20 object-contain sm:h-32' alt={'product image'} key={index}/>
        ))}
      </ScrollContainer>
    </div>
  )
