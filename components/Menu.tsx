    ...
    <div className={`w-3/4 h-screen z-40 bg-white absolute top-0 flex flex-col shadow-lg transition-all duration-300 right-0 overflow-hidden ${open ? ' ' : 'w-0'}`}>
        ...
        <div className='flex flex-col p-4 text-2xl gap-6 mt-12 font-semibold cursor-pointer'>
            <div className='border-b-2 py-2 pl-1 '>
                <div className='flex items-center hover:bg-[#f6f6f6]' onClick={() => setOpenCategories(!openCategories)}>
                    <p className='' >Categories</p>
                    <ChevronDownIcon className='h-8'/>
                </div>
                
                
                    <ul className={`${openCategories ? '' : 'h-0 hidden'} transition-all duration-300 h-40`}>
                        <li className='pl-4 py-1 border-b hover:bg-[#f6f6f6]' onClick={() => router.push('/mens')}>mens</li>
                        <li className='pl-4 py-1 border-b hover:bg-[#f6f6f6]' onClick={() => router.push('/womens')}>womens</li>
                        <li className='pl-4 py-1 border-b hover:bg-[#f6f6f6]' onClick={() => router.push('/jewelery')}>jewelery</li>
                        <li className='pl-4 py-1 hover:bg-[#f6f6f6]' onClick={() => router.push('/electronics')}>electronics</li>
                    </ul>
